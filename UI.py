...
class UI(pygame.sprite.Sprite):
    ...
    def putBoat(self, name, board, user="player"):
        ...
        if user == "player":
            ...
            tileSize = round((self.game.resolution[1] - 48) / self.game.computer_board.size)
            range = (self.game.resolution[0] / 2 - self.game.computer_board.size * tileSize / 2)
            boat.rect.x = randint(range, range + tileSize * (self.game.computer_board.size-1))
            boat.rect.y = randint(0, tileSize * (self.game.computer_board.size)-1)
