...
class UI(pygame.sprite.Sprite):
    ...
    def limitBoat(self, plateau, name):  # vérifie que la limite de bateau n'est pas atteinte
            ...
            if e.positioning:
                for t in self.game.all_Tiles:  # pour chaque case du plateau
                    if t.rect.collidepoint(e.rect.x, e.rect.y):  # si la souris est sur la case
                        ...
                        if self.game.board.size - t.coordonnee[
                            0] >= e.taille:  # vérifie qu'il y assez de case pour mettre le bateau
                        ...
                        if is_placeable:  # si le bateau n'est pas pas placable (donc est placable)
                            for t2 in self.game.board.all_tiles:  # pour chaque case ou est le bateau
                                if t2.coordonnee[1] == t.coordonnee[1]:
    ...
    def watching(self, event, mouse_x, mouse_y, check):
        if event.type == pygame.MOUSEBUTTONDOWN:  # si l'évènement est "Bouton de la souris pressé"
            ...
            if check == "positioning" and self.is_positioning:  # si on est train de positionner un bateau
                self.checkPlacementBoat()  # vérifie que le placement du bateau est correct
            ...
            if check == "button":
                ...
                for e in self.all_buttons:
                    ...
                    if e.rect.collidepoint(event.pos) and e.clicked:
                        ...
                        self.limitBoat("", e.name)  # vérifie si la limite de bateau n'est pas atteinte
                    else:
                        e.clicked = False
